...
function generateTimestampInLastWeek() {
  const now = Date.now();
...
async function seedDatabase() {
  ...
  try {
    ...
    // Display summary
    const totalDigests = await redis.get(STATS_KEYS.TOTAL_DIGESTS);
    ...
    console.log("\n🎯 Sample URLs seeded:");
    for (let i = 0; i < Math.min(5, recentUrls.length); i++) {
      const item =
        typeof recentUrls[i] === "string"
          ? JSON.parse(recentUrls[i])
          : recentUrls[i];
